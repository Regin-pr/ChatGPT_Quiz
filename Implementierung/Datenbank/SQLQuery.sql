...
--Die Datenbank behandelt Themen und Texte, die vom Nutzer eingegeben werden in Form von Einträgen in der "Thema_Text" Tabelle gleich
...
CREATE TABLE Frage(
	...
	Text nvarchar(200),
	--Schwierigkeit von 1-3 Leicht-Schwer (und vielleicht mehr) (so eingrenzen, dass nur diese Werte möglich sind)
	Schwierigkeit INT,
	Punktewert INT
)
...
Die Tiefsee ist gekennzeichnet durch eine Wassertemperatur von knapp 4 °C, Abwesenheit von Pflanzenwuchs und nahezu vollständige Dunkelheit. 
...
Die größte Tiefe, in der jemals ein Fisch beobachtet wurde, beträgt 8336 m (Stand April 2023). 
In dieser Tiefe wurde im September 2022 im Boningraben (auch Izu-Ogasawara-Graben) südlich von Japan ein einzelner Fisch aus der Familie der Scheibenbäuche (Liparidae) beobachtet. 
...
Damit überschreitet das beobachtete Exemplar die bisher für Fische angenommene maximal erreichbare Überlebenstiefe von 8200 m um 136 Meter');

INSERT INTO Frage VALUES
(1, 'Welche Farbe haben die Karten, die Schiedsrichter im Fußball zeigen, um einen Spieler zu verwarnen?', 1, 100),
(3, 'Welche ist die bisher größte beobachtete Tiefe, in der ein Fisch entdeckt wurde, und wo wurde diese Entdeckung gemacht?', 3, 500),
(2, 'Welche griechische Göttin ist die Göttin der Weisheit, des Krieges und der Strategie?', 2, 300);
